                    ...
                    <form method="post" id="mysimpan_data" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6" style="padding-top:20px;min-height:320px"> 
                            ...
                            <div class="form-group">
                                ...
                                <input type="text"  name="no_kwitansi" placeholder="No. Kwitansi / Memo Dinas" value="{{$data['no_kwitansi']}}"  class="form-control">
                            </div>
                            <div class="form-group">
                                <label>Nomor : PO/Kontrak/Ket.Pembayaran</label>
                                <input type="text"  name="no_po" placeholder="Nomor : PO/Kontrak/Ket.Pembayaran" value="{{$data['no_po']}}"  class="form-control">
                            </div>
                            <div class="form-group">
                                ...
                                <select name="mata_uang" style="width:20%;display:inline" class="form-control">
                                    ...
                                    @foreach(matauang() as $matauang)
                                        <option value="{{$matauang['name']}}" @if($matauang['name']==$data['mata_uang']) selected @endif>{{$matauang['name']}}</option>
                                    @endforeach
                                </select>
                                <input type="text" placeholder="Nilai Invoice / Memo Dinas" onkeyup="cek_nilai_kwitansi(this.value)" onkeypress="return hanyaAngka(event)" name="nilai_kwitansi" style="width:77%;display:inline" value="{{$data['nilai_kwitansi']}}" class="form-control">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="norek" style="width:20%;display:inline" class="form-control">
                                    ...
                                    @foreach(bank() as $bank)
                                        <option value="{{$bank['norek']}}" @if($bank['norek']==$data['norek']) selected @endif>{{$bank['norek']}}</option>
                                    @endforeach
                                </select>
                                <input type="text" name="nama_bank" placeholder="Nama Bank" value="{{$data['nama_bank']}}" style="width:77%;display:inline"  class="form-control">
                            </div>
                            <div class="form-group">
                                <label>Email / Telp / Fax Vendor</label>
                                <input type="text"  name="email" placeholder="Email / Telp / Fax Vendor"  value="{{$data['ket']}}" class="form-control">
                            </div>
                            <div class="form-group">
...
<script>
    ...
    function simpan_data(){
        ...
        var a="{{$ide}}";
            $.ajax({
                type: 'POST',
                url: "{{url('/bttd/simpan_edit/non_faktur')}}/"+a,
